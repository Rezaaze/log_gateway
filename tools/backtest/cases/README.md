...
|---|---|---|---|---|
| `pakistan-telecom-youtube-2008.toml` | ✅ verified (RIPE NCC's own case study — same data source this project reads) | ✅ downloaded once, real run (01.09.2026) — not kept in repo/committed (~1.8GB), re-download from `data.ris.ripe.net` to reproduce | ✅ built twice (60,127 entries at min_samples=30, 169,069 at min_samples=3) — not kept in repo | ✅ real run done, twice — see `TRUSTWAVE_ROADMAP.md` Abschnitt 3.2 for full numbers. Wave-physics: TPR 0.0% even after fixing the baseline-lookup fallback (origin_novelty + deaggregation_mismatch tiers, both unit-verified correct) — root cause confirmed as zero baseline coverage for this address space at any prefix length, itself caused by a stable/rarely-reannounced route almost never producing the synchronized multi-collector announce events this baseline methodology samples from (not fixable by lowering `min_samples`, verified). Simple `HijackDetector` layer: did fire, with a documented caveat (cold-start on a never-before-seen sub-prefix) |
